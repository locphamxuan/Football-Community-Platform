...
import { Badge, Button, Card, DetailRow, ErrorState, Loading, TextField } from '../components/ui';
...
export default function TeamDetailScreen({ teamId: id }: { teamId: string }) {
  const { user } = useAuth();
  ...
  if (isLoading) return <Loading label="Đang tải đội bóng" />;
  if (error || !data) {
    return <ErrorState message={messageOf(error)} onRetry={refetch} />;
  }
