...
**How to apply:** Đặt tên nhánh theo việc: `feature/...`, `fix/...`, `chore/...`, `docs/...`. Nhánh gốc thường là nhánh hiện tại nếu việc mới phụ thuộc code chưa merge, ngược lại thì từ `dev` — **không phải `main`**: `main` đứng yên từ một commit cũ (tháng 6), toàn bộ việc đang chạy thật nằm ở `dev` và các nhánh feature/security rẽ ra từ nó (xác nhận lúc 2026-08-21, `dev` khi đó đi trước `main` hàng chục commit). Không bao giờ commit trực tiếp lên `main` hay `dev`. Xem thêm [[feedback-commit-style]] và [[feedback-verify-before-commit]].
